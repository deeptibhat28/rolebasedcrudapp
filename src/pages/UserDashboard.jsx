...
return (
    <div className="min-h-screen w-full bg-[#1a0b2e] px-4 py-8 relative overflow-hidden text-white">
      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center bg-[#28133f] p-6 rounded-3xl shadow-xl mb-6 border border-purple-900/50 z-10 relative">
        <div className="mb-4 md:mb-0">
          <h1 className="text-2xl font-extrabold text-white">User Dashboard</h1>
          <p className="text-sm text-purple-200/70">Welcome <span className="font-semibold text-xl text-white">{currentUser?.username}</span></p>
        </div>
      ...
      <div className="max-w-6xl mx-auto bg-[#28133f] p-6 rounded-3xl shadow-xl border border-purple-900/50 z-10 relative">
        <h2 className="text-lg font-bold text-white mb-4">My Submissions</h2>

        {error && <p className="text-red-400 text-sm mb-4">{error}</p>}

        {submissions.length === 0 ? (
          <p className="text-purple-200/70 text-sm">You haven't added any submissions yet. Click "Create New Form" to get started.</p>
        ) : (
                ...
                {submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-purple-900/20 transition">
                    ...
                    <td className="py-3 px-2">
                      ...
                      <div className="text-xs text-purple-200/70">{sub.address || 'N/A'}</div>
                    </td>
                    <td className="py-3 px-2">
                      <div className="font-medium">{sub.department}</div>
                      <div className="text-xs text-purple-200/70">{sub.designation}</div>
                    </td>
